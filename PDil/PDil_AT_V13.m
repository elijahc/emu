%V11 - transitioning to a more put-together form of this task

sca; cl earvars; close all; clc;
RestrictKeysForKbCheck([])
rng('shuffle');% At this is recommended in the ^'s place


 TTLs = 'no'; %AT 2/25/20, this is  either 'no' or 'yes'
% TTLs = 'yes'; %AT 2/25/20, this is either 'no' or 'yes'

% practiceblock = 'no';
practiceblock = 'yes';

%AT 3/4/20 - change the below integer based on block #
BlockNumber = '0';


% % opponent = 'human'; %either 'human' or 'computer'; %This is f or purposes of the display screen only, you need to change the 'strategy' variable below if you actually wanna have a personVperson game
opponent = 'computer'; %either 'human' or 'computer'; %This is for purposes of the display screen only, you need to change the 'strategy' variable below if you actually wanna have a personVperson game


strategy = 'TT_coop'; % TT_coop/TT_defect/human; note if 'human' then Player B must input a keyboard response
% strategy = 'TT_defect'; % TT_coop/TT_defect/human; note if 'human' then Player B must input a keyboard response

% strategy = 'human'; % TT_coop/TT_defect/human; note if 'human' then Player B must input a keyboard response
...
%AT 3/4/20 - KEEP IN TESTRUN! even when using experiment
mode = 'testrun'; %either 'testrun' or 'experiment'
% mode = 'experiment'; %either 'testrun' or 'experiment'
...
if strcmp(mode, 'testrun')
    ...
    width = (rez.width)/1.075;
    height = (rez.height)/1.125;
...
for a = 1:nTrialsPerBlock
    ...
    if a == queryTime1 || a == queryTime2 || a == queryTime3
        ...
        RestrictKeysForKbCheck([Key_1, Key_2, Key_3, Key_4, Key_5, Key_6, Key_7, Key_8, Key_9])
    ...
    if a == queryTime1 || a == queryTime2 || a == queryTime3
        ...
        elseif a == queryTime3
            ...
            linea = '\n\n All done!';
            lineb = (['\n\n Your cumulative points: ' mat2str(sum(playerA_pts_summary))]);
...
% timing_sumTictocs_trial_n(queryTime3,1) = timing_sumTictocs_trial_n(queryTime3,1)+Time_query1(3)+Time_query1_rxntime(3)+Time_query1_Postrxntime(3)+Time_query2(3)+Time_query2_rxntime(3)+Time_query2_Postrxntime(3);
...
taskoutput.timing.durationWholeSession = wholesessionT;
...
if strcmp(practiceblock, 'no')
    ...
    taskoutput.timing.endScreen = endScreen;
...
taskoutput.timing.timing_sumTictocs_Intro = timing_sumTictocs_Intro;
taskoutput.timing.Introtime = Introtime;
taskoutput.timing.timing_wholesession_Intro = timing_wholesession_Intro;
...
if strcmp(practiceblock, 'no')
    save(strcat(['blockNum_', BlockNumber, '_', opponent, strategy, '_taskoutput.mat']), 'taskoutput')
elseif strcmp(practiceblock, 'yes')
    save(strcat(['PRACTICE_blockNum_', BlockNumber, '_taskoutput.mat']), 'taskoutput')
end
