...
FEDERATION_STARSHIPS = """name,full_name
...
def federation_starships(short=True):
    name_list = StringIO(FEDERATION_STARSHIPS)
    starships = pd.read_csv(name_list)
    if short:
        return list(starships.name.values)
    else:
        return starships
